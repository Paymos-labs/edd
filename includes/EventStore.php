...
final class EventStore implements EventStoreInterface
{
    ...
    public function remember($eventId, $ttlSeconds)
    {
        ...
        $key = 'paymos_edd_evt_' . md5((string) $eventId);
        $lockKey = $key . '_lock';
        ...
        if (function_exists('add_option')) {
            add_option('_transient_timeout_' . $lockKey, (string) (time() + 300), '', 'no');
            return add_option('_transient_' . $lockKey, '1', '', 'no');
        }
    ...
    private function releaseDatabase()
    {
        ...
        $hash = $this->pendingHash;
        if ($hash !== '' && self::canUseDatabase()) {
            $wpdb->delete(self::tableName(), array('event_hash' => $hash, 'status' => 'processing'), array('%s', '%s'));
        }
    ...
    private static function tableName()
    {
        ...
        return $prefix . 'paymos_edd_webhook_events';
    }
}
